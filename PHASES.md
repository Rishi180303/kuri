...
**Fixed the same day, live path only.** Until now the live CLI had no holiday detection: a holiday was processed as a trading day (NAV carried flat, a SUCCESS `daily_runs` row written) and counted toward the 20-day rebalance cadence — three such days so far (05-28, 06-26, 09-14) — and a rebalance landing on one would have traded against placeholder prices. `RunStatus.SKIPPED_HOLIDAY` existed, the cadence counter and the dashboard already honoured it, but nothing wrote it. `kuri papertrading run` now calls `skip_placeholder_holiday` first: if the target date has equity rows and none of them traded, the day closes as `skipped_holiday` with no portfolio row. One traded ticker is enough to call it a session (the 2025-03-18 shape), and a date with no rows at all is left alone because that is indistinguishable from a fetch outage.

Three things were deliberately left as they are. `run_daily` does not know about holidays, because the backfill replays 2026-01-15 and the frozen Phase 4 artifacts were computed with that placeholder day in them; parity still holds at 1.30e-15. The three holidays already processed keep their SUCCESS rows — the guard hands back any existing row rather than rewriting live history. And the placeholder rows stay in the raw store, so they still enter rolling features as zero-return, zero-volume days, which matches the data the folds were trained on; dropping them at ingest belongs with the retrain.
